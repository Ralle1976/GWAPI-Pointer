...
Func UpdateWorld(ByRef $aAgentArray, $aRange = 1350, $aMyID = GetMyID(), $aMe = GetAgentPtr($aMyID))
   ...
   $aRange = $aRange ^ 2
   If $MyID = 0 Then $MyID = $aMyID
   If $MyPtr = 0 Then $MyPtr = $aMe
   If GetIsDead($MyPtr) Then Return False
   If GetMapLoading() <> 1 Then Return True ; not explorable
   Local $lEnergyStruct = MemoryReadStruct($MyPtr + 284,'float EnergyPercent;long MaxEnergy')
   ...
   UpdateAgentPosByPtr($MyPtr, $lX, $lY)
   Local $lAgentEffects, $lAgentX, $lAgentY, $lAllegiance
	  ...
	  Switch $lAllegiance ; Allegiance
		 Case 1, 6 ;Allies
			...
			If Not BitAND($lAgentEffects, 0x0010) Then ; alive
			   ...
			   If $aAgentArray[$i] <> $MyPtr Then ; Other Allies
				  $mTeamOthers[0] += 1
