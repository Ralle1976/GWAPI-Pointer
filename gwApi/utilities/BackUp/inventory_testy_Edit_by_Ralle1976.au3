...
Func SalvageBags()
   ...
   EndIf
   ; Start processing
   Local $lBagPtr, $lItem, $lItemArrayPtr
   Local $lQuantity, $ItemMID, $ItemRarity, $ItemType
   Local $lMod, $lValue, $lUpgraded
   Local $lDeadlock, $lQuantityOld
   For $bag = 1 To 4 ; inventory only
	  $lBagPtr = GetBagPtr($bag)
	  If $lBagPtr = 0 Then ContinueLoop
	  $lItemArrayPtr = MemoryRead($lBagPtr + 24, 'ptr')
	  For $slot = 1 To MemoryRead($lBagPtr + 32, 'long')
		 ...
		 If $ItemRarity = 2621 Then
			For $i = 1 To $lQuantity
			   ...
			   EndIf
			   $lQuantityOld = $lQuantity
			   Update("Start Salvage (white): " & $bag & ", " & $slot & " -> " & StartSalvage($lItem, $lCheapKitID))
			   Local $lDeadlock = TimerInit()
			   Do
				  Sleep(250)
;~ 				  $lQuantity = MemoryRead($lItem + 75, 'byte')
		 ...
		 ElseIf $ItemRarity = 2623 Or $ItemRarity = 2626 Or $ItemRarity = 2624 Then ; blue or purple or gold
			$ItemType = MemoryRead($lItem + 32, 'byte')
			$lUpgraded = GetIsUpgraded($lItem)
			; armor salvage items
			If $lUpgraded And $ItemType = 0 Then
			   $lMod = Upgrades($lItem)
			   Local $lTimer = TimerInit()
			   While $lMod <> 0 And TimerDiff($lTimer) < 30000
				  If MemoryRead($lExpertKit + 12, 'ptr') = 0 Then
					 FindSalvKitEx($lExpertKit, 1, 16, True, False, True)
					 If $lExpertKit = 0 Then
						Return -1
				  ...
				  EndIf
				  $lValue = MemoryRead($lExpertKit + 36, 'short')
				  Update("Start Salvage Armor Mod: " & $bag & ", " & $slot & " -> " & StartSalvage($lItem, $lExpertKitID))
				  Sleep(1000 + GetPing())
				  Update("Salvage (" & $lMod - 1 & "): " & $bag & ", " & $slot & " -> " & SalvageMod($lMod - 1))
				  Local $lDeadlock = TimerInit()
				  Do
					 Sleep(250)
				  Until $SalvageState;$lValue <> MemoryRead($lExpertKit + 36, 'short') Or TimerDiff($lDeadlock) > 2500
				  SalvageOUT()
				  Sleep(250 + GetPing())
				  If MemoryRead($lItem + 12, 'ptr') = 0 Then ContinueLoop 2
				  $lMod = Upgrades($lItem)
			   WEnd
;~ 			; weapons
			ElseIf $lUpgraded And IsWeapon($ItemType) Then
			   $lMod = WeaponMods($lItem)
			   Local $lTimer = TimerInit()
			   While $lMod <> 0 And TimerDiff($lTimer) < 30000
				  If MemoryRead($lExpertKit + 12, 'ptr') = 0 Then
				  ...
				  EndIf
				  $lValue = MemoryRead($lExpertKit + 36, 'short')
				  Update("Start Salvage Weapon Mod: " & $bag & ", " & $slot & " -> " & StartSalvage($lItem, $lExpertKitID))
				  Sleep(1000 + GetPing())
				  Update("Salvage (" & $lMod - 1 & "): " & $bag & ", " & $slot & " -> " & SalvageMod($lMod - 1))
				  Local $lDeadlock = TimerInit()
				  Do
			...
			If $ItemRarity <> 2624 And MemoryRead($lItem + 12, 'ptr') <> 0 Then
			   If MemoryRead($lCheapKit + 12, 'ptr') = 0 Then
				  FindSalvKitEx($lCheapKit, 1, 16, True, True, False)
				  If $lCheapKit = 0 Then
					 Return -1 ; no more normal salvage kits
				  Else
					 $lCheapKitID = MemoryRead($lCheapKit, 'long')
				  EndIf
			   EndIf
			   Update("Start Salvage_: " & $bag & ", " & $slot & " -> " & StartSalvage($lItem, $lCheapKitID))
			   Sleep(1000 + GetPing())
			   If $ItemRarity <> 2623 And MemoryRead($lItem + 12, 'ptr') <> 0 Then
				  Update("Salvage (Materials): " & $bag & ", " & $slot & " -> " & SalvageMaterials())
				  $lDeadlock = TimerInit()
				  Do
					 Sleep(250)
				  Until $SalvageState;MemoryRead($lItem + 12, 'ptr') = 0 Or TimerDiff($lDeadlock) > 2500
...
Func Upgrades($aItemPtr)
   ...
   For $i =  0 to $numEnd
	  If ($Rune_Insigmia[$i][4] And StringInStr($lMod, $Rune_Insigmia[$i][0], 1) <> 0) Then
		 ConsoleWrite($lMod & " ==> " &  $Rune_Insigmia[$i][0] & @CRLF)
...
Func WeaponMods($aItemPtr)
   ...
   Local $numEnd = (UBound($Upgrades) -1)
   For $i = 0 To $numEnd
	   If (($Upgrades[$i][13] == 'True') And (StringInStr($lMod, $Upgrades[$i][0], 1) <> 0) And ($lType = $Upgrades[$i][10])) Then
		   ...
		   If (($Upgrades[$i][11] = 4) And ($_array[0] = $Upgrades[$i][8]) Or ($Upgrades[$i][12] = 2)  And ($_array[1] = $Upgrades[$i][9])) Then Return $Upgrades[$i][4];mod Pos
	   EndIf
   Next
   Return -1
EndFunc   ;==>WeaponMods
...
Func TestWeaponModsOrRunes()
Local Static $0 = '<c=@ItemUnique>'
Local Static $1 = '<c=@ItemEnhance>'
Local Static $2 = '<c=@ItemCommon>'
Local Static $3 = '<c=@ItemUncommon>'
Local Static $4 = '<c=@ItemRare>'
Local Static $5 = '</c>'
   For $bag = 1 To 4 ; inventory only
	  $lBagPtr = GetBagPtr($bag)
	  ConsoleWrite("Bag = " & $bag & @CRLF)
	  ConsoleWrite("$lBagPtr = " & $lBagPtr & @CRLF)
	  If $lBagPtr = 0 Then ContinueLoop
	  For $slot = 1 To MemoryRead($lBagPtr + 32, 'long') ; slots
		 $lItemPtr = GetItemPtrBySlot($lBagPtr, $slot)
		 ConsoleWrite("$lItemPtr = " & $lItemPtr & @CRLF)
		If $lItemPtr = 0 Then ContinueLoop
;~ 		If GetIsUnIDed($lItemPtr) Then
;~ 			IdentifyItem($lItemPtr)
;~ 			;ContinueLoop
;~ 		EndIf
		Local $NameString_ = MemoryRead($lItemPtr + 56, 'ptr')
		Local $lName = _GetItemString($NameString_);GetAgentName($lTemp);
		If StringInStr($lName, $0) Then $lName = StringReplace(_StringBetween($lName, $0, $5)[0], "[0-9]", "")
		If StringInStr($lName, $1) Then $lName = StringReplace(_StringBetween($lName, $1, $5)[0], "[0-9]", "")
		If StringInStr($lName, $2) Then $lName = StringReplace(_StringBetween($lName, $2, $5)[0], "[0-9]", "")
		If StringInStr($lName, $3) Then $lName = StringReplace(_StringBetween($lName, $3, $5)[0], "[0-9]", "")
		If StringInStr($lName, $4) Then $lName = StringReplace(_StringBetween($lName, $4, $5)[0], "[0-9]", "")

		Local $lRaity = MemoryRead($NameString_, 'ushort') ;
		Local $lType = MemoryRead($lItemPtr + 32, 'byte')
		Local $lModelID = MemoryRead($lItemPtr + 44, 'long')
		Local $e = MemoryRead($lItemPtr + 34, 'short')
		Local $lModStruct = MemoryReadStruct($lItemPtr + 16, 'ptr;long')
		Local $lMod = MemoryRead(DllStructGetData($lModStruct, 1), 'byte[' & DllStructGetData($lModStruct, 2) * 4 & ']')

		ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $lName = ' & " Bag = " & $bag & " Slot = " & $slot & "  " & $lName & "  " & $lRaity & "  " & $lType & "  " & $lModelID & "  " & $e _
		& " Modstruct = " &    $lMod  & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console ;& " WeaponMods = " & WeaponMods($lItemPtr) & "   RuneInsigmia = " & Upgrades($lItemPtr)
...
; "Attention this need a update to make settings array as a workable base
...
;~ Description: Returns array of agent ptrs, doesnt use ASM call.
;~ Author: Tecka, slight modifications to avoid caching problems.
Func MemoryReadAgentPtrStruct($aMode = 0, $aType = 0xDB, $aAllegiance = 3, $aDead = False)
