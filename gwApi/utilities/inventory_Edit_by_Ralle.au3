...
Func BuyIdentKit($aItemIDRow = 0, $aAmount = 1)
   ...
   ConsoleWrite("Try to buy Ident Kit! "  & @CRLF)
   Local $lItemIDRow, $lItemPtr, $lValue, $Price
   If $aItemIDRow = 0 Then
	  $lItemIDRow = GetItemRowByModelID(2989)
	  If $lItemIDRow = 0 Then
		 $lItemIDRow = GetItemRowByModelID(5899)
		 If $lItemIDRow = 0 Then  Return False ; no id kit
	  EndIf
   Else
   ...
   $Price =  ($aAmount * $lValue)
   Local $MoneyOnChar = GetGoldCharacter()
   ConsoleWrite("Prie are = " & $Price & @CRLF)
   If (($MoneyOnChar < $Price) And (GetGoldStorage() > ($Price - $MoneyOnChar))) Then
	    ConsoleWrite("Not enough Money..... take some !"  & @CRLF)
	   WithdrawGold($Price - $MoneyOnChar)
	   ConsoleWrite("Now is the Money on Char = " & GetGoldStorage()  & @CRLF)
	   If (GetGoldCharacter() < $Price) Then  Return False
   EndIf
...
Func BuySalvKit($aExpert = False, $aItemIDRow = 0, $aAmount = 1)
   If GetMapLoading() = 1 Then Return False
   ConsoleWrite("Try to buy SalvageKit! "  & @CRLF)
;~ Local Static $PriceForKit = [100,400,2000]
   Local $lItemIDRow, $lItemPtr, $lValue, $Price
   If $aItemIDRow = 0 Then
	  If $aExpert Then
		 $lItemIDRow = GetItemRowByModelID(2991)
   ...
   DllStructSetData($mBuyItem, 2, $aAmount)
   DllStructSetData($mBuyItem, 3, $lItemIDRow)
   DllStructSetData($mBuyItem, 4, $Price)
   Return SetExtended($lItemPtr, Enqueue($mBuyItemPtr, 16))
EndFunc   ;==>BuySalvKit
	  ...
	  For $slot = 0 To MemoryRead($lBagPtr + 32, 'long') - 1
		 ...
		 Switch $lItemMID
			Case 2992, 2993
				...
				If Not FindSlavageKitEx_HelpFunc1($aCheapKit, $lKitMID, $aPtr, $lItemPtr, $lUses, $lItemMID, 2) Then ExitLoop 2
			Case 2991
				If $aNormalOnly Then ContinueLoop
				If Not  FindSlavageKitEx_HelpFunc1($aCheapKit, $lKitMID, $aPtr, $lItemPtr, $lUses, $lItemMID, 8) Then ExitLoop 2
			Case 5900
				If $aNormalOnly Then ContinueLoop
				If Not FindSlavageKitEx_HelpFunc1($aCheapKit, $lKitMID, $aPtr, $lItemPtr, $lUses, $lItemMID, 10) Then ExitLoop 2
		 EndSwitch
...
Func FindSlavageKitEx_HelpFunc1($var_1, Byref $var_2, Byref $var_3, Byref $var_4, Byref $var_5, $lItemMID, $faktor)
Local $lValue = MemoryRead($var_4 + 36, 'short') / $faktor
If Not $var_1 Then
	FindSlavageKitEx_HelpFunc2($var_2, $var_3, $var_5, $var_4, $lValue, $lItemMID)
	Return False
Else
	If $lValue < $var_5 Then FindSlavageKitEx_HelpFunc2($var_2, $var_3, $var_5, $var_4, $lValue, $lItemMID)
EndIf
Return True
EndFunc

Func FindSlavageKitEx_HelpFunc2(Byref $var_2, Byref $var_3, Byref $var_5, $var_4, $lValue, $lItemMID)
$var_5 = $lValue
$var_3 = $var_4
$var_2 = $lItemMID
EndFunc
   ...
   For $bag = $aBagStart to $aBagEnd
	  ...
	  For $slot = 0 To MemoryRead($lBagPtr + 32, 'long') - 1
		 ...
		 Switch MemoryRead($lItemPtr + 44, 'long')
			Case 2989
			   If $aExpertOnly Then ContinueLoop
			   If Not FindIDKitEx_HelpFunc1($aCheapKit, $lKitMID, $aPtr, $lItemPtr, $lUses, True) Then ExitLoop 2
			Case 5899
			   If $aNormalOnly Then ContinueLoop
			   If Not FindIDKitEx_HelpFunc1($aCheapKit, $lKitMID, $aPtr, $lItemPtr, $lUses, False) Then ExitLoop 2
		 EndSwitch
		 If $lUses < 10 Then ExitLoop 2
	  Next
   Next
...
;$aCheapKit,$lKitMID,$aPtr,$lItemPtr,$lUses
Func FindIDKitEx_HelpFunc1($var_1, Byref $var_2, Byref $var_3, $var_4, Byref $var_5, $faktor = False)
Local $lValue = (MemoryRead($var_4 + 36, 'short') / (($faktor) ? (2) : (2.5)))
If Not $var_1 Then
	FindIDKitEx_HelpFunc2($var_2, $var_3, $var_5, $lValue, $var_4, $faktor)
	Return False
Else
	If $lValue < $var_5 Then
		FindIDKitEx_HelpFunc2($var_2, $var_3, $var_5, $lValue, $var_4, $faktor)
		Return False
	EndIf
EndIf
...
Func FindIDKitEx_HelpFunc2(Byref $var_2, Byref $var_3, Byref $var_5, $lValue, $var_4, $faktor)
$var_5 = $lValue
$var_3 = $var_4
$var_2 = (($faktor) ? (2989) : (5899))
EndFunc
#EndRegion
...
#Region New SalvageBags
Func IdentKitAction(ByRef $var_, $wichOne, $state1 = False, $State2 = False, $State3 = False)
FindIDKitEx($var_, 1, $wichOne, True)
If $var_ = 0 Then
	If Not BuyIdentKit() Then Return False
	FindIDKitEx($var_, 1, $wichOne, $state1)
EndIf
If $var_ = 0 Then  Return False
Return  True
EndFunc

Func SalvageKitAction(ByRef $var_, $wichOne, $state1 = False, $State2 = False, $State3 = False)
FindSalvKitEx($var_, 1, $wichOne, $state1, $State2, $State3)
If $var_ = 0 Then
	If Not BuySalvKit(True) Then Return False
	FindSalvKitEx($var_, 1, $wichOne, $state1, $State2, $State3)
	If $var_ = 0  Then Return False
EndIf
Return True
...
Func GetAvaibleBagPtr()
Local $lBagPtr = 0, $tempArray[16][2]
$tempArray[0][0] = 0
For $i = 1 To 4
	$lBagPtr = GetBagPtr($i)
	If $lBagPtr = 0 Then ContinueLoop
	$tempArray[0][0] += 1
	$tempArray[$tempArray[0][0]][0] = $lBagPtr
	$tempArray[$tempArray[0][0]][1] = $i ; are the Numer of the Bag
Next
Redim $tempArray[$tempArray[0][0] +1][2]
Return $tempArray
EndFunc
...
Local $MaxStorageOrBag = (($MapState = 0) ? (16) : (4))
; Search for ID kit
Local $lIDKit = 0,$lCheapKit = 0, $lExpertKit = 0

; Search for normal salvage kit
$lIdentify = IdentKitAction($lIDKit, $MaxStorageOrBag, True)
; Search for expert salvage kit
If Not SalvageKitAction($lExpertKit, $MaxStorageOrBag, True, False, True) Then Return False
; Search for cheapest salvage kit
If Not SalvageKitAction($lCheapKit, $MaxStorageOrBag, True, True) Then Return False
...
Local $lCheapKitID = MemoryRead($lCheapKit, 'long')
   ...
   ; Start processing
Local $lItem, $lItemArrayPtr, $lQuantity, $ItemMID, $ItemRarity, $lDeadlock, $lQuantityOld, $Disconnect = False, $MaxSolts = 0
Local $BagPtrArray = GetAvaibleBagPtr()
;~ _ArrayDisplay($BagPtrArray, "$BagPtrArray")
...
;~ 		If $globaldebug Then ConsoleWrite("Bag = " &  $BagPtrArray[$bag][1] & " Slot = " & $slot & " Last Slot (Max) = " &  $MaxSolts & @CRLF)
		$lItem = MemoryRead($lItemArrayPtr + 4 * ($slot - 1), 'ptr')
...
;~ 		; Identify item if necessary and id kit available
		If $lIdentify And GetIsUnIDed($lItem) Then
			...
			If MemoryRead($lIDKit + 12, 'ptr') = 0 Then
				$lIdentify = IdentKitAction($lIDKit, $MaxStorageOrBag, True)
				If Not $lIdentify Then ContinueLoop
				...
				Else
					...
					If $ItemRarity <> 2624 And MemoryRead($lItem + 12, 'ptr') <> 0 Then
						If MemoryRead($lCheapKit + 12, 'ptr') = 0 Then
							...
							$lCheapKitID = MemoryRead($lCheapKit, 'long')
						EndIf
						Update("Start Salvage_: " & $BagPtrArray[$bag][1] & ", " & $slot & " -> " & StartSalvage($lItem, $lCheapKitID))
						Sleep(1250 + GetPing())
...
Func CheckFuncStateAfterTrySalvage($state)
Local Static $String_Array = [4, " ",   "Deadlock trigger !", "by salvage i had a DC !", "salvage works !"]
	Switch($state)
		Case 3
...
For $___ModX = 1 To $____Mod[0][0]
	...
	EndIf
	Local $state_Of_StartSalvage = StartSalvage($lItem, $lExpertKitID);StartSalvageWithPacket($lItem, $lExpertKitID);
...
Func Upgrades($aItemPtr)
   ...
   Static $numEnd = (UBound($Rune_Insigmia) -1)
   For $i =  0 to $numEnd
	  If ($Rune_Insigmia[$i][4] And StringInStr($lMod, $Rune_Insigmia[$i][0], 1) <> 0) Then
		$TemReturn[0][0] += 1
		$TemReturn[0][1] = True
		ReDim $TemReturn[$TemReturn[0][0] +1][2]
		$TemReturn[$TemReturn[0][0]][0] =    $Rune_Insigmia[$i][1]
		$TemReturn[$TemReturn[0][0]][1] =   "Found In ==> " &$lMod & " This indetifier ==> " &  $Rune_Insigmia[$i][0] & " with the Name ==> " &  $Rune_Insigmia[$i][2]
	  EndIf
   Next
   Return $TemReturn
EndFunc   ;==>Upgrades
...
;~ 		_ArrayDisplay($_array, "StringRegExp from WeaponMods ! ")
		...
		$TemReturn[0][1] = True
		ReDim $TemReturn[$TemReturn[0][0] +1][2]
		$TemReturn[$TemReturn[0][0]][0] =  $Weapon_Mods[$i][4]
		$TemReturn[$TemReturn[0][0]][1] =   "Found In ==> " &$lMod & " This indetifier ==> " &  $Weapon_Mods[$i][0] & " with the Name ==> " &  $Weapon_Mods[$i][5]
	   EndIf
   Next
   Return $TemReturn
EndFunc   ;==>WeaponMods
   ...
   For $bag = 1 To 4 ; inventory only
	  ...
	  For $slot = 0 To MemoryRead($lBagPtr + 32, 'long') - 1
		 ...
		 ElseIf $ItemRarity = 2623 Or $ItemRarity = 2626 Then ; blue or purple
			...
			If (($ItemType = 0) And Upgrades($lItem)[0][1]) Then
			   ContinueLoop
			ElseIf (IsWeapon($ItemType) And WeaponMods($lItem)[0][1]) Then
			   ContinueLoop
			Else
   ...
   For $bag = 1 to 4 ; inventory only
	  ...
	  For $slot = 1 To MemoryRead($lBagPtr + 32, 'long')
		 ...
		 Switch MemoryRead($lItemPtr + 32, 'byte')
			Case 0
			   If Upgrades($lItemPtr)[0][1] Then ContinueLoop
			Case 2, 5, 12, 15, 19, 22, 24, 26, 27, 32, 35, 36
			   ...
			   If WeaponMods($lItemPtr)[0][1]  Then ContinueLoop
