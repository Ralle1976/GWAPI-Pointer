...
Func CacheSkillbar()
   ...
   Local $lSkillStruct1, $lSkillStruct2, $lEnergy
   For $i = 1 To 8
	  $mSkillbarCache[$i] = GetSkillbarSkillID($i)
	  $mSkillbarCachePtr[$i] = GetSkillPtr($mSkillbarCache[$i])
	  $lSkillStruct1 = MemoryReadStruct($mSkillbarCachePtr[$i] + 12, 'long Type;long Special;long Effect1;long Condition')
	  $lSkillStruct2 = MemoryReadStruct($mSkillbarCachePtr[$i] + 56, 'dword Adrenaline;float Activation;float Aftercast')
	  ...
	  Else
		 ...
		 $lEnergy = MemoryRead($mSkillbarCachePtr[$i] + 53, 'byte') ; EnergyCost
		 Switch $lEnergy
			Case 11
	  ...
	  EndIf
   Next
;~    Local $lMyProfession = GetHeroProfession(0)
;~    Local $lAttrPrimary = GetProfPrimaryAttribute($lMyProfession)
...
;~ 	  $mSkillbarPtr = GetSkillbarPtr()
   ...
   ; Skill usage
   Local $i, $IsAntiMeleeSkill, $IsHexingSpell, $IsConditioningSpell, $IsBelow50
